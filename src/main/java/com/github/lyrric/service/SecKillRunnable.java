        ...
        do {
            ...
            try {
                ...
                if (Config.orderId == null) {
                    String secKillResponse = httpService.secKill(vaccineId.toString(), "1", Config.memberId.toString(),
                            Config.idCard, Config.st);
                    try {
                        JSONObject resObject = JSONObject.parseObject(secKillResponse);
                        if (Config.orderId != null) {
                            logger.warn("Thread ID：{}: orderId {} is exist", id, resObject.getString("data"));
                        }
                        Config.orderId = resObject.getString("data");
                    } catch (Exception e) {
                        logger.error("", e);
                    }
                }
...
//                }
                ...
                if (System.currentTimeMillis() > startDate + 1000 * 60 *10) {
                    break;
                }
            }
        } while (true);
